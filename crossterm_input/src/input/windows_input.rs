...
fn read_single_event() -> Result<Option<InputEvent>> {
    ...
    match input.event_type {
        ...
        InputEventType::WindowBufferSizeEvent => return Ok(Some(InputEvent::Resize)),
        ...
        unsafe {
            match input.event_type {
                ...
                InputEventType::MouseEvent => {
                    ...
                    if let Some(event) = handle_mouse_event(&mouse_event) {
                        input_events.push(InputEvent::Mouse(event));
                    }
                }
                // NOTE (@imdaveho): ignore below
                InputEventType::WindowBufferSizeEvent => input_events.push(InputEvent::Resize),
...
fn handle_mouse_event(event: &MouseEvent) -> Option<super::MouseEvent> {
